...
def onConnect(client_4, userdata, flags, rc):
	if rc == 0:
...
def onMessage(client_4, userdata, msg):
	global waste_level4
	with lock:
		waste_level4 = 0
	print("bin_4 got empty")

######### mqttBroker = "demo.thingsboard.io" -> changed the solution arch 
mqttBroker = "localhost"

#create mqtt client named as waste_level4 --> we used here the paho.mqtt.client lib 
client_4 = mqtt.Client("waste_level4")
client_4.connect(mqttBroker, 1883, 60)
client_4.on_connect = onConnect

#------------------------------------------------- HERE WE will Start the subscribe to central station -----------------#
client_4.subscribe("central/empty4")
#some code to be done when it recieves msg from the broker
client_4.on_message = onMessage
...
while True:
	...
	with lock:
		waste_level4 = min( (waste_level4 + throw), 100 )	

	#to make the value published increase with only 10, I mean if it was increased by 1, the level still = 0
	waste_level_pub = round(waste_level4 / 10) * 10

	client_4.publish("bin_4/waste_level", waste_level_pub, 2, False)	
	...
	print("Just published " + str(waste_level_pub) + "% to TOPIC bin_4/waste_level")
	time.sleep(5)
